...
export default function RoomHeader({ room, memberCount, callerRole }: Props) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl px-6 py-5 mb-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <h1 className="text-2xl font-bold text-white truncate">{room.name}</h1>
          {room.description && (
        ...
        {canManage && (
          <Link
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold
              text-slate-400 bg-slate-800 hover:bg-slate-700 hover:text-white transition shrink-0"
          >
            <Settings size={13} />
            Settings
          </Link>
