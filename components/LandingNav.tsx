...
export default function LandingNav() {
  ...
  return (
    <nav className="relative flex items-center justify-between px-8 py-5 border-b border-slate-800/60">
      ...
      {/* Mobile dropdown */}
      {menuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-slate-900 border-t border-slate-800 px-6 py-4 flex flex-col gap-1 z-50">
          <Link href="/explore" onClick={() => setMenuOpen(false)}
            className="text-slate-300 hover:text-white text-sm py-2 transition-colors">
            Explore
          </Link>
          ...
          <Link href="/sign-up" onClick={() => setMenuOpen(false)}
            className="text-white font-semibold text-sm py-2 transition-colors">
            Get started →
          </Link>
        </div>
      )}
