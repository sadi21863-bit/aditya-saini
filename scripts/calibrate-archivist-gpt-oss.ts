/**
 * Step 0: Archivist calibration script — GPT-OSS 120B on Groq.
 *
 * CONTROLLED COMPARISON: identical to calibrate-archivist.ts in every way except:
 *   - Provider: Groq (not Cerebras — gpt-oss-120b is NOT available on this Cerebras account)
 *   - Model ID: "openai/gpt-oss-120b" (Groq's namespaced format)
 *
 * NOTE: GPT-OSS 120B on Groq emits <think> reasoning tags (confirmed in Week 2).
 * stripThinkingTags is applied to the response, matching the production callAgent path.
 *
 * Same system prompt, same synthetic data, same temperature, same max_tokens as original.
 *
 * Output is printed to console and saved to scripts/calibration-output-gpt-oss-groq.txt.
 *
 * Run: npx tsx scripts/calibrate-archivist-gpt-oss.ts
 */
...
async function main() {
  ...
  // ── Provider: Groq. Model: openai/gpt-oss-120b (Groq namespaced format).
  // gpt-oss-120b is NOT available on this Cerebras account (returns 404).
...
Write the archive narrative following your Archivist instructions. Output ONLY the JSON object.`;
  ...
  console.log("=== AI Lab Archivist Calibration — GPT-OSS 120B (Groq) ===");
  ...
  // GPT-OSS narrative is more verbose than Qwen — 2000 tokens was insufficient (truncated mid-JSON).
  // Bumped to 4000 to obtain a complete response. This token-budget difference is documented in
  // the comparison report as a relevant metric.
  const rawFromGroq = await callGroq(MODEL, PERSONA, userPrompt, { maxTokens: 4000, temperature: 0.7 });
  const elapsed  = Date.now() - start;
  // GPT-OSS 120B on Groq emits <think> tags (Week 2 finding) — strip before parsing
  const raw      = rawFromGroq;
  ...
  output.push(`=== ARCHIVIST CALIBRATION OUTPUT — GPT-OSS 120B (Groq) ===`);
  ...
  console.log(`\nSaved to scripts/calibration-output-gpt-oss-groq.txt`);
}

async function writeOutput(text: string) {
  const outPath = path.resolve(process.cwd(), "scripts", "calibration-output-gpt-oss-groq.txt");
