...
## Phase 2 — Multi-Round Debates (built 2026-05-21, extended 2026-08-10)
...
**Extended to N rounds (2026-08-10):** Max 3 rounds, max 3 pushbacks. User submits pushback text between rounds. When max rounds/pushbacks reached, verdict is auto-queued or user can request early verdict.

### New queue action: `debate_final_verdict`

Generated by `buildDebateVerdictPrompt` → `callGroq("openai/gpt-oss-20b")` with JSON mode. Returns `{ verdict, score, winner_id, summary }`. Writes `debates.verdict`, `debates.verdictReasoning`, `debates.winnerId`, sets `status=archived`.

### State machine

```
in_progress → awaiting_pushback → in_progress → ... → final_verdict → archived
```
...
|----------|--------|---------|
| `/api/debates/pushback` | POST | Store pushback text, queue next round |
| `/api/debates/[id]/verdict` | POST | Request early final verdict |

### New frontend components

| Component | Purpose |
|-----------|---------|
| `DebateRound` | Displays a single round's turns with agent names |
| `PushbackInput` | Text input for user pushbacks between rounds |
| `VerdictCard` | Shows final verdict (winner, summary, reasoning) |
| `RequestVerdictButton` | Button when max rounds/pushbacks reached |

### Updated file map

| File | Purpose |
|------|---------|
| `lib/agents/validators.ts` | `loadDebateState`, `canPushback`, `canTriggerVerdict` |
...
| `app/api/debates/pushback/route.ts` | POST — pushback endpoint |
| `app/api/debates/[id]/verdict/route.ts` | POST — verdict trigger endpoint |
| `components/debates/DebateRound.tsx` | Round display component |
| `components/debates/PushbackInput.tsx` | Pushback form component |
| `components/debates/VerdictCard.tsx` | Verdict display component |
| `components/debates/RequestVerdictButton.tsx` | Verdict request button |
| `drizzle/0015_multi_round_pushbacks.sql` | Migration — new table + columns |

## Phase 3 Scope (not yet built)

Personality overlays, additional modes (Error Check, Devil's Advocate, Build Roadmap), bridge statements / consensus points, argument graph, re-engagement banner, user-defined personas, "continue later" flow, `POST /api/debates/[id]/archive` manual trigger.
