    ...
    <div className="max-w-3xl mx-auto px-6 py-10">
      ...
      <header className="dark bg-[#1A1814] rounded-2xl p-6 mb-8">
        <p className="font-mono text-[11px] text-[#7A7268] uppercase tracking-widest mb-2">
          Monthly Retrospective · {formatMonth(yearmonth)}
        </p>
      ...
      {rollup.narrativeArc && (
        <section className="mb-8">
          <p className="font-mono text-[11px] uppercase tracking-widest text-ic-muted mb-4">Month in Review</p>
        ...
        <section className="mb-8">
          <details className="group">
            ...
            <ul className="mt-3 flex flex-col gap-2 pl-5">
              {weeklyRollups.map((r) => (
                <li key={String(r.periodEnd)} className="font-mono text-[12px]">
                  <Link href={`/ai-lab/archive/weekly/${String(r.periodEnd)}`} className="text-ic-accent hover:text-ic-accent-bright transition-colors">
      ...
      <div className="border-t border-ic-rule pt-8 flex items-center justify-between gap-4">
        {adjacent.prev ? (
          <Link href={`/ai-lab/archive/monthly/${adjacent.prev}`} className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-ic-rule text-ic-muted hover:border-ic-ink hover:text-ic-ink font-mono text-xs transition-colors">
            <ChevronLeft size={13} /> Previous month
          </Link>
        ) : <div />}
        ...
        {adjacent.next ? (
          <Link href={`/ai-lab/archive/monthly/${adjacent.next}`} className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-ic-rule text-ic-muted hover:border-ic-ink hover:text-ic-ink font-mono text-xs transition-colors">
