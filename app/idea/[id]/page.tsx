...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const ogUrl = `${baseUrl}/api/og?` + new URLSearchParams({
    title: idea.title ?? "",
  ...
  return {
    ...
    description,
    openGraph: { title: idea.title ?? "IdeaConnect", description, url: `${baseUrl}/idea/${ideaId}`, siteName: "IdeaConnect", images: [{ url: ogUrl, width: 1200, height: 630, alt: idea.title ?? "" }], type: "article" },
    twitter: { card: "summary_large_image", title: idea.title ?? "IdeaConnect", description, images: [ogUrl] },
  };
}
...
export default async function IdeaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [authorResult, likedResult, initialComments, viewerResult, reviewSummary] = await Promise.all([
    idea.userId ? getIdeaAuthor(idea.userId) : Promise.resolve(null),
    viewerId
      ? db.select({ id: ideaLikes.id }).from(ideaLikes).where(and(eq(ideaLikes.userId, viewerId), eq(ideaLikes.ideaId, ideaId))).limit(1)
      : Promise.resolve([]),
    getComments(ideaId),
    viewerId
      ? db.select({ xp: users.xp, name: users.name, handle: users.handle, image: users.image, tier: users.tier }).from(users).where(eq(users.id, viewerId)).limit(1)
      : Promise.resolve([]),
  ...
  return (
    <main className="min-h-screen bg-slate-950 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        <Link href="/feed" className="inline-flex items-center gap-2 text-slate-400 hover:text-[#0d9488] transition-colors font-semibold text-sm mb-8 group">
