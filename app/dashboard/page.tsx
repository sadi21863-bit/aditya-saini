...
export default async function DashboardPage({
  ...
  // Stats (always over all ideas)
  const allIdeas = await db.select().from(ideas).where(eq(ideas.userId, userId));
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="text-sm font-semibold text-[#0d9488] uppercase tracking-widest">My Workspace</p>
            </div>
          ...
          <Link
            href="/new"
            className="inline-flex items-center gap-2 bg-[#0d9488] text-white px-6 py-3 rounded-2xl font-bold hover:bg-teal-700 transition-all shadow-md active:scale-95 self-start md:self-auto"
          >
            <PlusCircle size={18} /> New Idea
          </Link>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div key={stat.label} className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
              <p className={`text-2xl font-bold ${stat.color}`} style={{ fontFamily: 'var(--font-playfair)' }}>
                {stat.value}
              </p>
          ...
          <div className="border-2 border-dashed border-slate-200 rounded-3xl p-20 text-center bg-white">
            <p className="text-slate-400 text-lg font-medium" style={{ fontFamily: 'var(--font-playfair)' }}>
              {tab === "drafts"   ? "No drafts yet." :
               tab === "launched" ? "Nothing launched yet. Publish a draft to see it here." :
               "Your workspace is empty. Create your first idea!"}
            </p>
            <Link href="/new" className="mt-6 inline-flex items-center gap-2 text-[#0d9488] font-bold hover:underline">
